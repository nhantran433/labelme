...
class Canvas(QtWidgets.QWidget):
    ...
    # Emitted when the user toggles shape visibility via Space key in edit mode.
    # Carries the list of shapes whose visibility changed.
    shape_visibility_toggled = QtCore.Signal(object)  # list[Shape]
    # Emitted when the user switches to edit mode via Ctrl+` keyboard shortcut.
    edit_mode_requested = QtCore.Signal()
    # Emitted when the shape type conversion was requested via Ctrl+T.
    shape_conversion_requested = QtCore.Signal()
    ...
    def _bounded_move_vertex(
        ...
        if shape.shape_type == "rectangle" and len(shape.points) == 4:
            # Keep the rectangle axis-aligned when dragging a corner
            # Points: [0=tl, 1=tr, 2=br, 3=bl]
            # We fix the opposite corner and let the dragged corner define new bounds
            opp = (vertex_index + 2) % 4  # opposite corner index
            fx = float(shape.points[opp][0])  # fixed opposite x
            fy = float(shape.points[opp][1])  # fixed opposite y
            new_x = pos.x()
            new_y = pos.y()
            x1 = min(fx, new_x)
            y1 = min(fy, new_y)
            ...
            if self.selected_shapes:
                ...
                for shape in self.selected_shapes:
                    if len(shape.points) >= 2:
                        ...
                        # Set to 4-point rectangle
                        while len(shape.points) < 4:
                            shape.points = np.append(
                                shape.points,
                                [[0, 0]],
                                axis=0,
                            )
                        shape.points = shape.points[:4]
                        shape.points[0] = [x1, y1]
                        shape.points[1] = [x2, y1]
                        shape.points[2] = [x2, y2]
                        shape.points[3] = [x1, y2]
                ...
                if changed:
                    ...
                    self.backup_shapes()
                    self.update()
                    self.shape_conversion_requested.emit()
        self._update_status()
...
def _reproject_oriented_rectangle_corners(
    *,
    ...
    # _drag_shapes saves the cursor offset in _prev_point, so a single-vertex
    # drag looks like a shape translation and every vertex moves together.
    # Compute step from the dragged vertex's previous position.
    step = pos - corners[vertex_index]

    if not allow_out_of_bounds:
        for corner in (c0, c1, c2, c3):
            dest = corner + step
            if _is_out_of_image(dest, image_size):
                return corners

    if vertex_index % 2 == 0:
        # vertices 0 and 2 are opposite; 1 and 3 are opposite.
        c0 = c0 + step
        c2 = c2 + step
    else:
        c1 = c1 + step
