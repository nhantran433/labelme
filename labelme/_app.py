...
""",
    ...
    def _restore_last_file(self) -> None:
        """After loading a directory, restore the last opened file."""
        last_file = self._window_state.value("lastImagePath", "", type=str)
        if not last_file or not QtCore.QFile.exists(last_file):
            return
        if last_file not in self.image_list:
            return
        index = self.image_list.index(last_file)
        self._docks.file_list.setCurrentRow(index)
    ...
    def _setup_canvas(self) -> _CanvasWidgets:
        ...
        canvas.shape_visibility_toggled.connect(self._on_shape_visibility_toggled)
        canvas.edit_mode_requested.connect(self._on_edit_mode_requested)
        canvas.shape_conversion_requested.connect(self._on_shape_conversion_requested)
    ...
    def _update_file_dock_title(self) -> None:
        ...
        if count > 0 and current >= 0:
            self._docks.file_dock.setWindowTitle(
                self.tr("File List (%d/%d)") % (current + 1, count)
            )
        else:
            self._docks.file_dock.setWindowTitle(self.tr("File List"))
    ...
    def _on_shape_visibility_toggled(self, shapes: list[Shape]) -> None:
        # Space key toggled visibility: update label list checkboxes
        ...
        for shape in shapes:
            ...
            if item is not None:
                with QtCore.QSignalBlocker(self._docks.label_list._model):
                    item.setCheckState(
                        Qt.CheckState.Checked if shape.visible else Qt.CheckState.Unchecked
                    )
    ...
    def _on_edit_mode_requested(self) -> None:
        # Ctrl+` was pressed: re-sync action states for edit mode
        self._switch_canvas_mode(edit=True)

    def _on_shape_conversion_requested(self) -> None:
        # Ctrl+T was pressed: re-sync label list items for the converted shapes
        self.mark_dirty()
        self._canvas_widgets.canvas.update()
    ...
    def closeEvent(self, a0: QtGui.QCloseEvent) -> None:
        if not self._can_continue():
            a0.ignore()
        if self._image_path:
            self._window_state.setValue("lastImagePath", self._image_path)
